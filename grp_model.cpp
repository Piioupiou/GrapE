...
namespace grp {
    ...
    GrpModel::GrpModel(grp::GrpDevice &device, const std::vector<Vertex> &vertices) : grpDevice(device) {
        createVertexBuffers(vertices);
    }

    GrpModel::~GrpModel() {
        vkDestroyBuffer(grpDevice.device(), vertexBuffer, nullptr);
        vkFreeMemory(grpDevice.device(), vertexBufferMemory, nullptr);
    }
    ...
    void GrpModel::createVertexBuffers(const std::vector<Vertex> &vertices) {
        ...
        void *data;
        vkMapMemory(grpDevice.device(), vertexBufferMemory, 0, bufferSize, 0, &data);
        memcpy(data, vertices.data(), static_cast<size_t>(bufferSize));
        vkUnmapMemory(grpDevice.device(), vertexBufferMemory);
    }
